...
class SteamDBParser:
    ...
    async def process_batch_async(self, context, batch: List[int]):
        ...
        try:
            # Parse CCU data through Compare
            ...
            for app_id, ccu_data in ccu_results.items():
                if ccu_data:
                    self.database.save_ccu_data(app_id, ccu_data)
            ...
            for app_id in batch:
                self.checkpoint_manager.mark_app_error(app_id, 'ccu', str(e),
                                                      f"{config.STEAMDB_COMPARE_URL}{','.join(map(str, batch))}")
    ...
    async def run_async(self):
        ...
        logger.info(f"Starting parsing for {len(pending_app_ids)} APP IDs")
        ...
        batch_manager = BatchManager(pending_app_ids, config.COMPARE_BATCH_SIZE)
        
        # Process batches sequentially to maintain session continuity
        # Using one context for all batches to preserve Cloudflare session
        processed_batches = 0
        
        # Get a single context that will be reused for all batches
