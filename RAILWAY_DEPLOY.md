...
Парсер теперь работает через REST API! Вы можете управлять им удаленно через HTTP запросы.

**Основные endpoints:**
- `POST /start` - запуск парсера с файлом app_ids
- `GET /status` - проверка статуса парсинга
...
3. Подключите ваш GitHub репозиторий: `https://github.com/progprogect/steamids-parser.git`
...
6. Добавьте переменные окружения (опционально):
   ...
   - `PORT=5000` (Railway автоматически установит)
...
После развертывания ваш парсер будет доступен по адресу: `https://your-app.railway.app`

### Запуск парсера через API

```bash
# Загрузите файл с app_ids
...
## Экспорт данных (альтернативные методы)

### Вариант 1: Через API (рекомендуется)

Используйте `/export` endpoint как показано выше.

### Вариант 2: Экспорт из PostgreSQL через Railway CLI
